...
每当 Scratch 执行任何积木时，它必须做很多工作：
 ...
 - 等等。即使只执行单个积木，Scratch 中也有*很多*事情在发生。

解释器开销是在 JavaScript 本身的开销之上增加的。由于此代码涉及许多动态类型，JavaScript JIT 可能难以优化它。

TurboWarp 的编译器通过将脚本直接转换为 JavaScript 函数来消除所有这些开销。例如，上述脚本变成：
...
当然，这是一个非常简单的脚本，解释器开销可以忽略不计，对于大多数项目来说也是如此。只有当你每帧执行数千个积木时，解释器的开销才会变得显著。
