...
基于函数的帽子积木允许你创建类似于以下内容的东西：

![](./assets/forever-if-something-then.svg)

基于函数的帽子基本上是事件积木的更强大版本。它使用相同的 startHats 并支持 `shouldRestartExistingThreads`。除此之外，帽子积木必须定义一个返回 true 或 false 的函数。积木的所有输入都将被评估，然后帽子积木可以使用该信息来确定是否应该运行其下方的积木。积木类型是 `Scratch.BlockType.HAT`。我们可以将上面的内容近似为：
...
这里有一个棘手的事情是 Scratch 不会自动启动基于函数的帽子积木——你需要自己做。在此示例中，我们使用 `BEFORE_EXECUTE` 事件(顾名思义，它在运行任何脚本之前运行，所以你在这里启动的任何东西都将在该帧中运行)。与事件积木一样，你可以从任何获得回调的地方运行基于函数的帽子积木。

## 边缘激活的帽子积木

基于函数的帽子允许你在条件*为*真时运行脚本。边缘激活的帽子积木允许你在条件*变为*真时运行脚本。
...
与基于函数的帽子类似，边缘激活的帽子可以接受任意输入，如果需要，也可以返回 Promise。
